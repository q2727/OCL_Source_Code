...
**核心问题**: 如何从分类数据中**自动**学到一个有语义意义的值顺序，构建合理距离度量？
...
> 📄 Figure 2 对应**外循环**（order learning 过程: 值密度→排序→线性组合）；Figure 3 对应**内外循环结构**（蓝三角=外循环 order update，两个蓝三角之间=内循环收敛，红方块=最终收敛点）
...
**结论**: OCL 全面超过传统 KMD（CA +0.064, ARI +0.076）——自动学到的 value order 有效弥补了 Hamming 距离丢失语义的缺陷（→ Slide 4 问题）
...
**OCL 优点**:
- 自动学习 value order，不需要领域知识（→ 直接回应 Slide 4 的核心问题）
...
- 需要预知 k（真实类别数）
- Order learning 可能学到反效果顺序（HR 实测 CA 0.36 < KMD 0.38）
